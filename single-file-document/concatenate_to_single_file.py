...
def replace_jekyll_tags_for_variables(doc_body, config):
    doc_body = doc_body.replace("{{ site.current_imagec_hash }}",         config["current_imagec_hash"])
    doc_body = doc_body.replace("{{ site.current_imagec_version }}",      config["current_imagec_version"])
    doc_body = doc_body.replace("{{ site.current_short_imagec_version }}", config["current_short_imagec_version"])
    return doc_body
...
def replace_box_names(doc_body):
    ...
    doc_body = doc_body.replace("> Underconstruction",   "> **Under construction.**")
    return doc_body
...
def adjust_links_in_doc_body(doc_body):
    ...
    doc_body = re.sub(
            r"\[installation page\]\((.*)?\)",
            r"[installation page](https://imagec.org/docs/installation/)",
            doc_body
        )
    ...
    doc_body = doc_body.replace(
        "]({% link docs/stable/tutorials/overview.md %}#python-client)",
    ...
    doc_body = doc_body.replace("](/data/", "](https://imagec.org/data/")
...
def change_links(doc_body):
    ...
    for match in matches:
        ...
        if original_link.startswith("/"):
            full_url_link = f"https://imagec.org{original_link}"
...
with open("../_config.yml") as config_file, open("metadata/metadata.yaml", "w") as metadata_file:
    ...
    metadata_file.write(textwrap.dedent(
        f"""
          ---
          title: ImageC Documentation
          subtitle: >-
            ImageC version {config["current_snapshot_version"]}\\newline
            Generated on {datetime.now(timezone.utc).strftime("%Y-%m-%d at %H:%M UTC")}
          ---
        """))

# compile concatenated document
with open(f"imagec-doc.md", "w") as of:
